...
public class Bus implements Serializable, Comparable<Bus> {
	...
	public Object[] toArray()
	{
		...
		{
			Object[] array = {id, driver.getName() + driver.getLastName(), plate, route.getName(), nextStopStation.getName(), state, UtilCalc.round(speed * 3.6, 2) , UtilCalc.round(position, 2), nextNode, movementState, UtilCalc.round(acceleration, 2)};
			return array;
		}
		else
		{
			if (driver != null) {
				Object[] array = {id, driver.getName() + driver.getLastName(), plate, route.getName(), "Unknowed", state, UtilCalc.round(speed * 3.6, 2) , UtilCalc.round(position, 2), nextNode, movementState, UtilCalc.round(acceleration, 2)};
				return array;
			}
			else
			{
				Object[] array = {id, "Not asigned", plate, route.getName(), nextStopStation.getName(), state, UtilCalc.round(speed * 3.6, 2) , UtilCalc.round(position, 2), nextNode, movementState, UtilCalc.round(acceleration, 2)};
				return array;
			}
		}
	}
