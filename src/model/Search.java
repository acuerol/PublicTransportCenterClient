...
public class Search implements Serializable {
	 ...
	 *            the searhed node.
	 * @param visited
	 *            the list of visit nodes.
	 */
	public void search(Graph graph, Object start, Object searched,
	...
	private void searchPaths(Graph graph, Object start, Object searched,
		...
		if (visited.get(visited.size() - 1) instanceof Station) {
			nodes = ((Station) (visited.get(visited.size() - 1)))
					.getChildNodes();
		} else {
			nodes = ((Semaphore) (visited.get(visited.size() - 1)))
					.getChildNodes();
		}
	 ...
	 * 
	 * @param stopStations
	 *            the stop stations for filter paths.
	 * @return the select path.
	 */
	public ArrayList<Object> selectByStopStationsPath(
			ArrayList<Station> stopStations) {
		if (paths.size() == 0) {
			...
			if (paths.size() == 1) {
				...
				do {
					...
					if (Util.isInteger(opt)) {
						...
						System.err
								.println("The number isn't a integer. Try again.");
						val = false;
					}
				} while (!val);
