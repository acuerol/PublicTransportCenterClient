...
public class PublicTransportCenter implements Serializable {
	...
	/**
	 * Return the PublicTransportCenter instance, if it is null then instance
	 * this, if it isn null return the instance.
	 * 
	...
	public static synchronized void setPublicTransportCenter(
			PublicTransportCenter pTC) {
		PublicTransportCenter.pTC = pTC;
	}

	public static synchronized void refreshBusesFromServer(
			PublicTransportCenter pTC) {
		...
		for (Bus bus : buses) {
			...
			if (index == -1) {
				...
				PublicTransportCenter.pTC.getBuses().get(index)
						.setAcceleration(bus.getAcceleration());
				PublicTransportCenter.pTC.getBuses().get(index)
						.setNextStopStation(bus.getNextStopStation());
				PublicTransportCenter.pTC.getBuses().get(index)
						.setNextNode(bus.getNextNode());
				PublicTransportCenter.pTC.getBuses().get(index)
						.setMovementState(bus.getMovementState());
			}
		}
	...
	public static synchronized void refreshBusesFromClient(
			PublicTransportCenter pTC) {
		ArrayList<Bus> buses = pTC.getBuses();
		int index = 0;
	...
	public double getDistanceAB(ArrayList<Object> path) {
		...
		distance = UtilCalc.round(distance, 3);
	...
	/**
	 * Searches in the roads the start and final point of each station and
	 * semaphore.
	 */
	public void loadNodesAttached() {
		ArrayList<Object> nodesAttached = new ArrayList<Object>();
	 ...
	 * 
	 * @param buses
	 *            all buses to the start at system.
	 */
	 ...
	 * 
	 * @param routes
	 *            all routes to the start at system.
	 */
	 ...
	 * 
	 * @param stations
	 *            all stations to start at system.
	 */
