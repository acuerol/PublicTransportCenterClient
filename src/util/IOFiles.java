...
public class IOFiles {
	...
	public static ArrayList<Road> loadRoads(String pathName) {
		...
		try {
			if (!file.exists()) {
				System.err.println("Roads: The path " + pathName
						+ "don't exist.");
				return null;
			}
			...
			while ((line = bufferedR.readLine()) != null) {
				...
				startPoint = new Point2D.Double(
						java.lang.Double.parseDouble(tempPoint[0]),
						java.lang.Double.parseDouble(tempPoint[1]));
	...
	public static ArrayList<Route> loadRoute(String pathName ) {
		...
		try {
			if (!file.exists()) {
				System.err.println("Route: The path " + pathName
						+ "don't exist.");
				return null;
			}
	...
	public static ArrayList<Semaphore> loadSemaphores(String pathName) {
		...
		try {
			if (!file.exists()) {
				System.err.println("Semaphores: The path " + pathName
						+ "don't exist.");
				return null;
			}
			...
			while ((line = bufferedR.readLine()) != null) {
				tokenizer = new StringTokenizer(line, ";");
				name = tokenizer.nextToken();
				timeRed = (int) java.lang.Double.parseDouble(tokenizer
						.nextToken());
				timeGreen = (int) java.lang.Double.parseDouble(tokenizer
						.nextToken());
				...
				semaphores
						.add(new Semaphore(name, timeRed, timeGreen, position));
			}
	...
	public static ArrayList<Station> loadStations(String pathName) {
		...
		try {
			if (!file.exists()) {
				System.err.println("Stations: The path " + pathName
						+ "don't exist.");
				return null;
			}
	...
	private static Way loadWay(ArrayList<Station> stopStations) {
		...
		pTC.getSearch().search(pTC.getGraph(), stopStations.get(0),
				stopStations.get(stopStations.size() - 1), null);

		pathRoute = pTC.getSearch().selectByStopStationsPath(stopStations);

		for (int i = 0; i < pathRoute.size(); i++) {
			distances.add(pTC.getDistanceAB(new ArrayList<Object>(pathRoute
					.subList(0, i))));
		}
	 ...
	 *            the file path in the disk.
	 * @param names
	 *            the list with all semaphores names.
	 * @param times
	 *            the vector with the two semaphores times, timeGreen and
	 *            timeRed.
	 * @param coordinates
	...
	public static boolean writeSemaphoresFile(String pathName,
		...
		try {
			...
			for (int i = 0; i < names.size(); i++) {
				bufferedW.append(names.get(i) + ";" + times.get(i)[0] + ";"
						+ times.get(i)[1] + ";" + coordinates.get(i)[0] + ";"
						+ coordinates.get(i)[1]);
				bufferedW.newLine();
			}
	 ...
	 *            the file path in the disk.
	 * @param sNames
	 *            the list with the all stations names.
	 * @param sCoordinates
	 *            the list with the all stations coordinates.
	 * @param sStates
	...
	public static boolean writeStationsFile(String pathName,
		...
		try {
			...
			for (int i = 0; i < sNames.size(); i++) {
				...
				state = sStates.get(i);
				bufferedW.append(name + ";" + coordinates[0] + ";"
						+ coordinates[1] + ";" + state);
				bufferedW.newLine();
			}
